...
def test_client():
    ...
    The client must be used as a context manager: only then does Starlette run
    the application lifespan, and this app loads the model in its startup
    event. Without the `with` block every prediction request returns 503.
    """
